...
鲸鱼保留完整的填充点阵，仅去除孤立噪点，并以鼠标跟随光照保持白点可见。安装卡片和选中标签使用 24px 背景模糊；入场动画结束后必须释放父层滤镜，否则磨砂效果无法采样卡片后方的背景。

首屏背景只使用参考站的纵向渐隐蒙版，不叠加额外深色渐变；流体背景以 1.8 秒模糊淡入，结束后释放滤镜，减少动态效果模式下不播放入场动画。底部社区区域保留独立遮罩。

导航 Harness 徽标采用双层圆角结构：外层为 135 度四段高光渐变，内层为 25% 黑色背景和 11px Fragment Mono 字体；随 HTML 预渲染，不依赖客户端挂载后补齐样式。
